...
//******************************************************************************************************************************************
void fillDisplay2(unsigned char param ,unsigned int column, unsigned int row) {
  unsigned char page, x;

  dataBuffer = malloc(column+1);
  dataBuffer[0] = SSD1306_DATA_MODE;
  for (page = 0; page < row; page++) {
      setCursor(0, page);
    for (x = 0; x < column; x++) {
        dataBuffer[x + 1] = param;
    }
    sendData(dataBuffer, column+1);
  }
  free(dataBuffer);
}
//******************************************************************************************************************************************
...
void displayDataToOled(const unsigned char rType, char* oledData){
	char wheatherArr[] = "Temper. : |A.Pres. : |Humidity: |W. Seed : |Cloudiness: |Status  : ";
	char serverArr[] = "Up Time : |S.Temp. : |Client  : ";
	char timeArr[] = "Date : |Time : ";

    char *oledDataPtr, *infoDataPtr;
    oledDataPtr = oledData;
    ...
    switch(rType){
		case 'T':
			...
			break;
		case 'W':
			draw12x16Str(16,i, "Weather", 1);
			infoDataPtr = wheatherArr;
			break;
		case 'S':
			draw12x16Str(40,i, "Server", 1);
			infoDataPtr = serverArr;
			break;
    }

    while( ((oledDataToken = strsep(&oledDataPtr,"|")) != NULL ) && ((infoDataToken = strsep(&infoDataPtr,"|")) != NULL)){
    	sprintf(screenData,"%s %s", infoDataToken, oledDataToken);
    	switch(rType){
    		case 'T':
    			draw12x16Str(k,i, oledDataToken, 1);
    			i += 16;
    			k += 16;
    			break;
    		case 'W':
    			draw6x8Str(0,(i+2), screenData, 1, 0);
    			i += 1;
    			break;
    		case 'S':
    			draw6x8Str(0,(i+2), screenData, 1, 0);
